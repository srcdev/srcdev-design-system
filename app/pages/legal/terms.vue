<template>
  <NuxtLayout name="default">
    <template #layout-content>
      <LayoutRow tag="div" variant="full-width" :style-class-passthrough="['mbe-24']">
        ...
        <div>
          <section v-for="item in termOfUseData" :key="item.link" :id="item.link">
            <article v-for="(section, articleIndex) in item.section" :key="articleIndex">
              <h3 :class="{ 'page-heading-2': Number(articleIndex) === 0, 'page-heading-3': Number(articleIndex) > 0 }">
                {{ section.title }}
              </h3>

              <div v-for="(content, contentIndex) in section.content" :key="contentIndex">
                <ul v-if="Array.isArray(content)">
                  <li v-for="(listItem, listIndex) in content" :key="listIndex" v-html="renderMarkdown(listItem)"></li>
                </ul>
